...
INPUT_STYLE_SET = ['event_type', 'event_type_sent', 'keywords', 'triggers', 'template', 'na_token']
...
ROLE_PH_MAP = {
    ...
    'ROLE_Thing': 'something', # ERE
    'ROLE_Audience': 'some publicity', # ERE
}
TRIGGER_PH_MAP = {
    'Trigger': '<Trigger>'
}
        ...
        if instance_base:
            for e_type in self.vocab['event_type_itos']:
                self.event_templates.append(
                    ...
                    self.input_style, self.output_style, passage, self.events)
                )
        else:
            for event in self.events:
                self.event_templates.append(
                    event_template(event['event type'], patterns[dataset][event['event type']], 
                    self.input_style, self.output_style, event['tokens'],  event)
                )
...
class event_template():
    def __init__(self, event_type, info_dict, input_style, output_style, passage, gold_event=None):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'trigger:sentence':
                    output_template += ' {} {}'.format(SEP, format_template(self.info_dict['ED template']))
                if o_style == 'argument:sentence':
                    output_template += ' {} {}'.format(SEP, format_template(self.info_dict['EAE template']))
    ...
    def generate_input_str_detail(self, query_trigger):
        ...
        for i_style in INPUT_STYLE_SET:
            if i_style in self.input_style:
                ...
                if i_style == 'na_token':
                    input_str += ' {} {} {} {}'.format(SEP, f"{AND}", "<sep>", f"{PAD}")
                ...
                if o_style == 'argument:sentence':
                    ...
                    for argu in self.arguments:
                        ...
                        for role_type in roles:
                            filler['{}{}'.format(ROLE_TEMPLATE_PREFIX, role_type)] = f" {AND} ".join([ a['argument text'] for a in argu[role_type]]) if role_type in argu.keys() else ROLE_PH_MAP['ROLE_{}'.format(role_type)]
                                ...
                                for idx in range(len(contexts)-1):
                                    try:
                                        ...
                                        for argu in pred_argu:
                                            if argu != ROLE_PH_MAP["{}{}".format(ROLE_TEMPLATE_PREFIX, roles[idx])]:
                                                if argu != '':
